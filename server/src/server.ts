		...
		return getConversationInfoByConversationId(conversation_id)
			.then((conv: { org_id: any; zid: any }) => {
				return getXidRecordByXidOwnerId(
					...
					// Look up participant if exists to avoid duplicate creation attempts
					return getParticipant(conv.zid, uidForCurrentUser)
						.then((ptpt: any) => {
							if (ptpt && ptpt.pid) {
								assigner(req, "pid", ptpt.pid);
							}
							onDone();
						})
						.catch((err: any) => {
							// Participant doesn't exist yet, that's ok - will be created on first vote/comment
							logger.debug("No participant found for xid user", { xid, uid: uidForCurrentUser, zid: conv.zid });
							onDone();
						});
				});
			})
